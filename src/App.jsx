...
import { Play, Info, Flame, Home, Search, ChevronLeft, Menu, X, Star, Clock, ListVideo, MonitorPlay } from 'lucide-react';
                ...
                return (
                  <button
                    ...
                    className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                      isActive ? 'bg-slate-800 text-rose-500' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
                    }`}
                  >
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 left-2 flex flex-col gap-1">
        {drama.isCompleted === "1" && (
          <span className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-white bg-rose-600 rounded">Tamat</span>
        )}
      ...
      <div className="absolute bottom-0 left-0 w-full p-4 transform translate-y-2 group-hover:translate-y-0 transition-transform">
        <h3 className="text-white font-bold text-sm md:text-base line-clamp-2 leading-tight mb-1">{drama.title}</h3>
        <div className="flex items-center text-xs text-slate-400 gap-3 opacity-0 group-hover:opacity-100 transition-opacity delay-100">
          <span className="flex items-center"><Play className="w-3 h-3 mr-1" /> {(drama.viewCount / 1000).toFixed(1)}k</span>
...
const SkeletonGrid = () => (
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
    {[...Array(12)].map((_, i) => (
      <div key={i} className="aspect-[3/4] rounded-xl bg-slate-800 animate-pulse"></div>
    ))}
  </div>
);
...
const HomeView = () => {
  ...
  if (loading) return <div className="pt-20 px-4 max-w-7xl mx-auto"><SkeletonGrid /></div>;
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 md:p-16 max-w-7xl mx-auto flex flex-col justify-end h-full">
            <div className="flex flex-wrap gap-2 mb-3">
              {heroDrama.categoryNames?.slice(0, 3).map(cat => (
                <span key={cat} className="px-2 py-1 text-xs font-semibold text-rose-400 bg-rose-500/10 rounded-full border border-rose-500/20">
                  {cat}
                </span>
              ))}
            </div>
...
const HotView = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (data && data.data) {
        setCategories(data.data.filter(cat => cat.recommendModules && cat.recommendModules.length > 0));
      }
      setLoading(false);
    };
    loadData();
  ...
  if (loading) return <div className="pt-24 px-4 max-w-7xl mx-auto"><SkeletonGrid /></div>;
    ...
    <div className="pt-24 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-2 text-sm">Drama pilihan khusus untuk 18+</p>
        </div>
      </div>
...
const DetailView = ({ dramaId }) => {
  ...
  if (!detail) return <div className="pt-24 text-center text-white">Data tidak ditemukan.</div>;
        ...
        <div className="absolute bottom-0 w-full">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 flex flex-col md:flex-row gap-8 items-end relative z-10">
            <div className="w-32 md:w-56 flex-shrink-0 rounded-xl overflow-hidden shadow-2xl shadow-black border border-slate-700/50 hidden md:block">
              <img src={detail.posterImgUrl || detail.posterImg} alt={detail.title} className="w-full h-auto" />
            </div>
            
            <div className="flex-1 w-full text-left">
              <button onClick={() => window.history.back()} className="mb-4 flex items-center text-slate-400 hover:text-white transition-colors">
                <ChevronLeft className="w-5 h-5 mr-1" /> Kembali
              </button>
              
              <div className="flex flex-wrap gap-2 mb-3">
                {detail.categories?.map(cat => (
                  <span key={cat.id} className="px-3 py-1 text-xs font-semibold text-rose-100 bg-rose-600/30 rounded-full border border-rose-500/30 backdrop-blur-sm">
                    {cat.name}
                  </span>
                ))}
              </div>
              
              <h1 className="text-3xl md:text-5xl font-extrabold text-white mb-4 leading-tight">{detail.title}</h1>
              
              <div className="flex flex-wrap items-center text-sm text-slate-300 gap-4 mb-6">
                <span className="flex items-center"><ListVideo className="w-4 h-4 mr-1 text-rose-500"/> {detail.totalEpisodes} Episode</span>
                <span className="flex items-center"><Clock className="w-4 h-4 mr-1 text-rose-500"/> {detail.publishedAt?.split(' ')[0]}</span>
                <span className="flex items-center"><Play className="w-4 h-4 mr-1 text-rose-500"/> {detail.viewCount?.toLocaleString()} Views</span>
              </div>
              
              <div className="flex gap-4">
                {detail.episodes && detail.episodes.length > 0 ? (
                  <button 
                    onClick={() => navigate(`/watch/${dramaId}/${detail.episodes[0].episodeNumber}`)}
                  ...
                  >
                    <Play className="w-5 h-5 mr-2 fill-current" /> Mulai Menonton
                  </button>
                ) : (
                   <button disabled className="flex-1 md:flex-none px-8 py-3.5 bg-slate-800 text-slate-500 rounded-lg font-bold cursor-not-allowed">
                     Episode Kosong
                   </button>
                )}
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 flex flex-col lg:flex-row gap-10">
        ...
        <div className="lg:w-2/3">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-slate-400 text-sm">{detail.episodes?.length || 0} Episode Tersedia</span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {detail.episodes?.map((ep) => (
              <button
                key={ep.id}
                onClick={() => navigate(`/watch/${dramaId}/${ep.episodeNumber}`)}
...
const WatchView = ({ dramaId, initialEpNum }) => {
  ...
  useEffect(() => {
    ...
    const loadVideo = async () => {
      ...
      const episode = detail.episodes?.find(e => parseInt(e.episodeNumber) === currentEpNum);
      ...
      if (episode && episode.fileId) {
        ...
        if (vData && vData.Result && vData.Result.PlayInfoList) {
          const sortedList = vData.Result.PlayInfoList.sort((a, b) => b.Bitrate - a.Bitrate);
  ...
  useEffect(() => {
    ...
    const processSubtitles = async () => {
        ...
        const tracks = await Promise.all(videoData.subtitleTracks.map(async (track) => {
            try {
    ...
    if (subtitlesReady && videoRef.current) {
        ...
        if (playPromise !== undefined) {
            playPromise.catch(error => {
                console.log("Autoplay dicegah oleh browser, abaikan. User perlu klik Play.", error);
            });
        }
    }
  }, [subtitlesReady, videoData]);
  ...
  if (!detail && loading) return (
    <div className="pt-24 flex justify-center"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-rose-500"></div></div>
  );
    ...
    <div className="pt-16 min-h-screen bg-slate-950 flex flex-col lg:flex-row">
      <div className="w-full lg:w-[70%] xl:w-[75%] flex flex-col">
        <div className="w-full bg-black aspect-video relative group">
          {videoLoading || (videoData && !subtitlesReady) ? (
            <div className="absolute inset-0 flex items-center justify-center bg-slate-900 flex-col z-10">
               <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-rose-500 mb-3"></div>
               <span className="text-slate-400 font-medium text-sm">Memuat Video & Subtitle...</span>
            </div>
        ...
        <div className="p-4 md:p-6 bg-slate-900 border-b border-slate-800 flex-shrink-0">
          <div className="flex flex-col md:flex-row justify-between md:items-start gap-4">
            <div>
              <button onClick={() => navigate(`/detail/${dramaId}`)} className="text-rose-500 text-sm font-semibold hover:underline mb-1">
                {detail?.title}
              </button>
      ...
      <div className="w-full lg:w-[30%] xl:w-[25%] bg-slate-900 flex flex-col h-[50vh] lg:h-[calc(100vh-64px)] border-l border-slate-800">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
          ...
          <span className="text-xs text-slate-400 bg-slate-800 px-2 py-1 rounded">{detail?.episodes?.length || 0} Eps</span>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans selection:bg-rose-500/30">
      ...
      <main className="min-h-screen">
        ...
        {route.path === '/watch' && <WatchView dramaId={route.params.id} initialEpNum={route.params.ep} />}
      </main>
        ...
        <footer className="bg-slate-950 border-t border-slate-900 py-8 text-center text-slate-500 text-sm">
          <div className="max-w-7xl mx-auto px-4 flex flex-col items-center">
             <div className="flex items-center mb-4">
                ...
                <span className="text-lg font-bold text-slate-700">DramaNova</span>
             </div>
